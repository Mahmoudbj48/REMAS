...
class ApplyLearningAgent:
    ...
    def load_recommendations(self, csv_file: str) -> List[Dict]:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(recommendations)} recommendations from {csv_file}"
            )
    ...
    def filter_high_confidence_recommendations(
        ...
        high_confidence = [
            r for r in recommendations if r["confidence"] >= self.confidence_threshold
        ]

        self.logger.info(
            f"Found {len(high_confidence)} high-confidence recommendations (>= {self.confidence_threshold})"
        )
        self.logger.info(
            f"Filtered out {len(recommendations) - len(high_confidence)} low-confidence recommendations"
        )

        return high_confidence

    def validate_parameter_changes(
        self, recommendations: List[Dict], current_weights: Dict[str, float]
    ) -> List[Dict]:
        ...
        for rec in recommendations:
            ...
            if param_name in [
                ...
                if 0.0 <= suggested_value <= 1.0:
                    ...
                    self.logger.warning(
                        f"Rejecting {param_name}: value {suggested_value} out of bounds [0,1]"
                    )
                ...
                if 1 <= suggested_value <= 20:
                    ...
                    self.logger.warning(
                        f"Rejecting {param_name}: value {suggested_value} out of bounds [1,20]"
                    )
                ...
                if 0.0 <= suggested_value <= 0.5:
                    ...
                    self.logger.warning(
                        f"Rejecting {param_name}: value {suggested_value} out of bounds [0,0.5]"
                    )
        ...
        self.logger.info(
            f"Validated {len(valid_recommendations)} out of {len(recommendations)} recommendations"
        )
    ...
    def apply_recommendations(self, recommendations: List[Dict]) -> Dict[str, Any]:
        ...
        valid_recommendations = self.validate_parameter_changes(
            high_confidence, current_weights
        )
        ...
        for rec in valid_recommendations:
            ...
            self.logger.info(
                f"Applied: {param_name} {old_value:.3f} → {new_value:.3f} (Δ{new_value-old_value:+.3f})"
            )
    ...
    def show_pending_recommendations(self) -> bool:
        ...
        print(
            f"High confidence (>= {self.confidence_threshold}): {len(high_confidence)}"
        )

        if high_confidence:
            print("\nHigh Confidence Changes:")
            print(
                f"{'Parameter':<25} {'Current':<10} {'Suggested':<10} {'Change':<10} {'Confidence':<10}"
            )
            print("-" * 75)
            ...
            for rec in high_confidence:
                ...
                print(
                    f"{param:<25} {current:<10.3f} {suggested:<10.3f} {change:+<10.3f} {confidence:<10.3f}"
                )
        ...
        if choice == "1":
            ...
            if result["success"]:
                ...
                for change in result["changes_applied"]:
                    print(
                        f"  {change['parameter']}: {change['old_value']:.3f} → {change['new_value']:.3f}"
                    )
